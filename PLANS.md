...
2. 通读并整理 2011、2013、2015、2018、2020 的 codebook、用户手册、问卷、发布说明、流程图和血检/体检专项文档，形成“文档用途 -> 对数据处理的指示 -> 对 FI/CAD 建模的影响”的摘要。
3. 基于 metadata/codebook 生成变量字典和安全别名方案：保留原始 Stata 列名作为机器键，另行提供解释名、中文名或 label-friendly 视图，避免直接覆盖 CSV 原列名造成跨波次对齐失败。
