...
- 正在做：进入第二轮建模改进；已确认上一版 `height_cm_2011` 错把中位数约 43 的 `qh006` 当作身高优先来源，导致“身高重要性异常靠前”的解释性结论不可靠。
- 下一步：重新构建数据集和模型 run，强制安装 SHAP 等解释性依赖，输出所有图表底层 CSV、可疑特征诊断、敏感性特征集对比和最新 ZIP 分享包。
