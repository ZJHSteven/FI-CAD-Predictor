...
- `data/curated` 中每张 `.dta` 都对应 3 个输出：CSV、Parquet、metadata JSON。
...
frame = pd.read_parquet("data/curated/2013-wave2/Health_Status_and_Functioning/Health_Status_and_Functioning.parquet")
```
...
metadata = json.loads(Path("data/curated/2013-wave2/Health_Status_and_Functioning/Health_Status_and_Functioning.metadata.json").read_text(encoding="utf-8"))
